...
class SyncDatabaseProcess
{
     ...
     * 一次批量导入最大数量
     */
    const BATCH_INSERT_ROWS_NUMBER = 50;
     ...
     * @param int   $pageSize
     */
    public function __construct(array $from, array $to, $pageSize = 1000)
    {
    ...
    public function sync(array $tableConf)
    {
        ...
        $sql = sprintf("UPDATE t_sync_tables SET sync_state=1, sync_date='%s' WHERE table_name='%s' AND sync_state=0", date('Y-m-d'), $tableName);
        $this->to->exec($sql);
        ...
        $sql = sprintf("UPDATE t_sync_tables SET sync_state=2, sync_time=%s WHERE table_name='%s' AND sync_state=1", $time, $tableName);
        $this->to->exec($sql);
        if($tableName == 't_company_user') {

        }
        $this->print('【' . $tableName . '】数据导入完毕，耗时：' . $time);
    }
    ...
    {
        $this->to->exec(sprintf('DROP TABLE IF EXISTS `%s`', $tableName));
...
//        $toStmt = $this->to->query(sprintf('SHOW TABLES LIKE "%s"', $tableName));
